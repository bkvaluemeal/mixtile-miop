...
struct miop_pcie {
	...
	u64 peer_db_off[4];
	dma_addr_t peer_db_buf[4];
	void __iomem *peer_data_base[4];
	dma_addr_t peer_data_dma[4];
	dma_addr_t peer_data_src[4];
	dma_addr_t peer_pci_dma[4];
	dma_addr_t peer_pci_elbi[4];
	int link_slot;
	u32 serial;
	int link_up;
...
/* Maps one per-peer outbound iATU window (target = 0x90000000 + peer<<25),
 * ioremaps it, returns VA; *out_phys gets the window's fabric address. */
